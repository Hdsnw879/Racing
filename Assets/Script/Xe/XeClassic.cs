...
public class XeClassic : MonoBehaviour
{
  // Start is called before the first frame update
    ...
    //Car Audio
    private AudioSource carAudio,carSkid;
    private float minPitch =0.1f;
    // Settings
    private float acceleration = 350, brakeForce = 300, maxSteerAngle = 10;
    private GameObject wCollider,effect,meshes,colliders;
    ...
    private void GetObject()
    {
        ...
        wCollider = transform.Find("Wheels").gameObject;
        
        //Get Wheel Collider from Game Object 
        colliders = wCollider.transform.Find("Colliders").gameObject;
        wheelCollider[0] = colliders.transform.Find("Wheel_FL").gameObject.GetComponent<WheelCollider>();
        wheelCollider[1] = colliders.transform.Find("Wheel_FR").gameObject.GetComponent<WheelCollider>();
        wheelCollider[2] = colliders.transform.Find("Wheel_BL").gameObject.GetComponent<WheelCollider>();
        wheelCollider[3] = colliders.transform.Find("Wheel_BR").gameObject.GetComponent<WheelCollider>();
        //Get Wheel Mesh 
        meshes = wCollider.transform.Find("Meshes").gameObject;
        wheelMesh[0] = meshes.transform.Find("WHEEL_FL").transform;
        wheelMesh[1] = meshes.transform.Find("WHEEL_FR").transform;
        wheelMesh[2] = meshes.transform.Find("WHEEL_RL").transform;
        wheelMesh[3] = meshes.transform.Find("WHEEL_RR").transform;
        //Get Particle
        ...
        // boost[0] = effect.transform.Find("boost").gameObject.GetComponent<ParticleSystem>();
        // boost[1] = effect.transform.Find("boost2").gameObject.GetComponent<ParticleSystem>();
    ...
    private void Acceleration() 
    {
        if(wheelCollider[0].isGrounded && inputManager.verticalInput != 0){
            ...
            wheelCollider[0].motorTorque = currentAcceleration + 100;
            wheelCollider[1].motorTorque = currentAcceleration + 100;
            wheelCollider[2].motorTorque = currentAcceleration - 100;
            wheelCollider[3].motorTorque = currentAcceleration - 100;
        }else
        {
    ...
    void BoostUp()
    {
        if(inputManager.isBoost)
        {
            foreach(var boost in boost)
            {
                boost.Play();
            }
